...
import static org.openstreetmap.josm.plugins.mdbuildings.BuildingsTags.DEFAULT_COMMON_BUILDING_VALUES;
...
public final class BuildingsSettings {
    private BuildingsSettings() {
    }

    static final String SETTING_PREFIX = "mdbuildings.";
    ...
    public static final BooleanProperty USE_DTM_ELEVATION = new BooleanProperty(
        SETTING_PREFIX + "use_dtm_elevation", true
    );

    public static final BooleanProperty MERGE_INTERIOR_ADDRESS_NODES = new BooleanProperty(
        SETTING_PREFIX + "merge_interior_address_nodes", false
    );
    
    public static final StringProperty DEFAULT_SOURCE = new StringProperty(
        SETTING_PREFIX + "default_source", "AGCC"
    );

    public static final StringProperty DATA_SOURCE_SERVERS = new StringProperty(
        SETTING_PREFIX + "data_source_servers",
        DataSourceServer.toJson(List.of(
            new DataSourceServer("GEODATA", "https://geodata.gov.md/geoserver/maps/wfs"),
            new DataSourceServer("CADASTRU", "https://cadastru.md/geoserver/w_cbi/wfs")
        )).toString()
    );

    public static final StringProperty DATA_SOURCE_PROFILES = new StringProperty(
        SETTING_PREFIX + "data_source_profiles",
        DataSourceProfile.toJson(List.of(
            new DataSourceProfile(
                "GEODATA", "maps:lm17_area_roof_area", "maps:lm17_area_roof_area", "GEODATA (Roofs)",
                true, "", "layer_name,layer,medium,source:building"
            ),
            new DataSourceProfile(
                "CADASTRU", "w_cbi:cad_cladiri", "w_cbi:cad_cladiri", "CADASTRU (Buildings)",
                true, "NRCASA", ""
            )
        )).toString()
    );
    ...
    public static final ListProperty CURRENT_DATA_SOURCE_PROFILE = new ListProperty(
        SETTING_PREFIX + "current_data_source_profile", new ArrayList<>(List.of("GEODATA", "GEODATA (Roofs)"))
    );
    ...
    public static final ListProperty UNWANTED_SOURCE_VALUES = new ListProperty(
        SETTING_PREFIX + "unwanted_source_values", new ArrayList<>(List.of("bing", "yahoo"))
    );
